...
var validation = (function (){
  ...
  var init = function(){
      ...
      validateForm = function (form) { // Проверяет, чтобы все поля формы были не пустыми. Если пустые - вызывает тултипы
        ...
        var elements = form.find('input, textarea').not('input[type="hidden"], input[disabled]'),
            valid = true;
      ...
      _setUpListners = function () { // Прослушивает все события 
        $('form').on('input', '.has-error', _removeError);
      ...
      _clearForm = function(form) { // Очищает форму 
        ...
        var form = $(this);
        form.find('.input, .textarea, .fileupload').trigger('hideTooltip'); // удаляем тултипы
